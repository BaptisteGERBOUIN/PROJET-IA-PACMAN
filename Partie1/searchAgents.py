...
class CornersProblem(search.SearchProblem):
    ...
    def expand(self, state):
        ...
        for action in self.getActions(state):
            nextPosition = self.getNextState(state, action)
            ...
            nextState = (nextPosition, tuple(nextCornersVisited))
...
def cornersHeuristic(state, problem):
    ...
    # Si tous les coins ont été visités, l'heuristique est 0 (état but atteint)
    if not remaining_corners:
        return 0

    # Calcul de la distance totale estimée pour couvrir tous les coins restants
    total_distance = 0
    current = current_position

    while remaining_corners:
        # Trouver le coin le plus proche du point de départ actuel
        closest_corner, min_distance = min(
            ((corner, util.manhattanDistance(current, corner)) for corner in remaining_corners),
            key=lambda x: x[1]
        )
        
        # Ajouter la distance vers le coin le plus proche
        total_distance += min_distance
        # Mettre à jour la position actuelle et enlever le coin visité
...
def foodHeuristic(state, problem):
    ...
    # Si toute la nourriture a été mangée, l'heuristique est 0 (état but atteint)
    if not food_positions:
        return 0

    # Calcul de la distance de Pacman à chaque point de nourriture
    max_distance = 0
    for food in food_positions:
        distance = util.manhattanDistance(position, food)
        max_distance = max(max_distance, distance)

    # Estimation supplémentaire: distance maximale entre les points de nourriture
    max_food_pair_distance = 0
    for i, food1 in enumerate(food_positions):
        for food2 in food_positions[i+1:]:
            pair_distance = util.manhattanDistance(food1, food2)
            max_food_pair_distance = max(max_food_pair_distance, pair_distance)

    # Combinez les deux valeurs pour obtenir une estimation réaliste du parcours nécessaire
    heuristic_value = max(max_distance, max_food_pair_distance)
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        # Use A* search to find the path to the closest food
